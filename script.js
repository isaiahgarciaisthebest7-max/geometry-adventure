...
// --- COLORS (Authentic to GD Levels) ---
const BG_COLORS = [
    '#2E64FE', '#D32EFE', '#00FF40', '#FF0000', '#0080FF', 
    '#8000FF', '#FF0040', '#FF8000', '#FF00FF', '#000080', 
    '#2E2E2E', '#804000', '#FF4000', '#202020', '#6600cc', 
    '#3498db', '#d35400', '#2c3e50', '#27ae60', '#c0392b', 
    '#8e44ad', '#e67e22', '#00c040', '#f1c40f', '#ffaa00'
];
...
const Gen = {
    ...
    // 1. Classic Cube (Stereo Madness / Back on Track)
    cubeWalk: function(length, type) {
        let arr = [];
        for (let i = 0; i < length; i++) {
            ...
            if (type === 'stereo') {
                if (i%30 === 0) { arr.push({x: cx, y: 1, t: 1}); arr.push({x: cx+1, y: 2, t: 1}); }
                if (i%60 === 30) { arr.push({x: cx, y: 1, t: 2}); arr.push({x: cx+1, y: 1, t: 2}); arr.push({x: cx+2, y: 1, t: 2}); }
            } else if (type === 'basic') {
                if (i%20 === 0) arr.push({x: cx, y: 1, t: 2});
            }
        }
        this.add(arr);
    },

    // 2. Platforms (Jump Pads sim)
    cubePlatforms: function(count) {
        ...
        for (let i = 0; i < count; i++) {
            this.cursorX += 160;
            let cx = this.cursorX/40;
            arr.push({x: cx, y: 0, t: 2}); // Floor spike
    ...
    // 4. Ball Section (Physically Correct)
    ...
    // 5. Spider Sim (Tight Ball/Robot)
    spiderSim: function(length) {
        let arr = [];
        for (let i = 0; i < length; i++) {
            this.cursorX += 40;
            let cx = this.cursorX/40;
            // Tight floor and ceiling to force rapid switching
            arr.push({x: cx, y: 0, t: 1}); arr.push({x: cx, y: 4, t: 1});
            if (i % 10 === 0) {
                if (Math.random() > 0.5) arr.push({x: cx, y: 1, t: 2});
                else arr.push({x: cx, y: 3, t: 2});
            }
        }
        this.add(arr);
    },

    // 6. Swing Copter Sim (Open Ship with Center Pillars)
    swingSim: function(length) {
        let arr = [];
        for (let i = 0; i < length; i++) {
            this.cursorX += 40;
            let cx = this.cursorX/40;
            if (i % 15 === 0) {
                // Floating center pillar
                arr.push({x: cx, y: 7, t: 1});
                arr.push({x: cx, y: 8, t: 1});
                arr.push({x: cx, y: 6, t: 2}); // Spike Bottom
                arr.push({x: cx, y: 9, t: 2}); // Spike Top
            }
        }
        this.add(arr);
    },
...
function buildLevel(index) {
    ...
    // START POSITION OVERRIDES
    if (index === 2) Gen.add([{x:10,y:4,t:1}]); // Polargeist High Start
    if (index === 19) Gen.add([{x:10,y:5,t:7}]); // Deadlocked starts Wave
    
    switch(index) {
        // 1. STEREO MADNESS
        case 0: Gen.cubeWalk(200, 'stereo'); Gen.portal(3); Gen.shipTunnel(300, 7); Gen.portal(4); Gen.cubeWalk(200, 'stereo'); break;
        // 2. BACK ON TRACK
        case 1: Gen.cubeWalk(50, 'basic'); Gen.cubePlatforms(15); Gen.portal(3); Gen.shipTunnel(400, 6); Gen.portal(4); Gen.cubePlatforms(10); break;
        // 3. POLARGEIST
        case 2: Gen.cubeWalk(150, 'basic'); Gen.portal(3); Gen.shipTunnel(300, 6); Gen.portal(4); Gen.cubeWalk(200, 'stereo'); break;
        // 4. DRY OUT (Gravity)
        case 3: Gen.cubeWalk(100, 'basic'); Gen.portal(5); Gen.ballSection(400); Gen.portal(4); Gen.cubeWalk(200, 'basic'); break;
        // 5. BASE AFTER BASE
        case 4: Gen.cubeWalk(100, 'basic'); Gen.portal(3); Gen.shipTunnel(600, 6); Gen.portal(4); Gen.cubeWalk(100, 'basic'); break;
        // 6. CAN'T LET GO
        case 5: Gen.cubePlatforms(20); Gen.portal(5); Gen.ballSection(500); Gen.portal(4); Gen.cubeWalk(200, 'stereo'); break;
        // 7. JUMPER
        case 6: Gen.portal(8); Gen.cubeWalk(400, 'basic'); Gen.portal(3); Gen.shipTunnel(300, 5); Gen.portal(4); Gen.cubeWalk(200, 'basic'); break;
        // 8. TIME MACHINE
        case 7: Gen.cubeWalk(100, 'stereo'); Gen.portal(5); Gen.ballSection(400); Gen.portal(8); Gen.cubeWalk(300, 'basic'); break;
        // 9. CYCLES
        case 8: Gen.portal(5); Gen.ballSection(300); Gen.portal(6); Gen.ufoSection(15); Gen.portal(5); Gen.ballSection(300); break;
        // 10. XSTEP
        case 9: Gen.cubeWalk(100, 'basic'); Gen.portal(3); Gen.shipTunnel(300, 5); Gen.portal(5); Gen.ballSection(300); Gen.portal(6); Gen.ufoSection(10); break;
        // 11. CLUTTERFUNK
        case 10: Gen.cubePlatforms(25); Gen.portal(3); Gen.shipTunnel(500, 4); Gen.portal(5); Gen.ballSection(400); break;
        // 12. ToE
        case 11: Gen.cubeWalk(100, 'basic'); Gen.portal(6); Gen.ufoSection(20); Gen.portal(3); Gen.shipTunnel(400, 4); Gen.portal(4); Gen.cubeWalk(200, 'basic'); break;
        // 13. ELECTROMAN
        case 12: Gen.cubeWalk(200, 'stereo'); Gen.portal(3); Gen.shipTunnel(400, 4); Gen.portal(4); Gen.cubePlatforms(20); Gen.portal(6); Gen.ufoSection(20); break;
        // 14. CLUBSTEP
        case 13: Gen.portal(3); Gen.shipTunnel(500, 4); Gen.portal(5); Gen.ballSection(400); Gen.portal(6); Gen.ufoSection(25); Gen.portal(8); Gen.cubeWalk(300, 'stereo'); break;
        // 15. ELECTRODYNAMIX
        case 14: Gen.cubeWalk(100, 'basic'); Gen.portal(3); Gen.shipTunnel(400, 3); Gen.portal(6); Gen.ufoSection(30); Gen.portal(4); Gen.cubePlatforms(30); break;
        // 16. HEXAGON FORCE (Slope Sim)
        case 15: Gen.cubeWalk(200, 'stereo'); Gen.portal(5); Gen.ballSection(400); Gen.portal(3); Gen.shipTunnel(300, 4); Gen.portal(4); Gen.cubeWalk(200, 'basic'); break;
        // 17. BLAST PROCESSING (Wave)
        case 16: Gen.portal(7); Gen.waveSection(800, 5); Gen.portal(4); Gen.cubeWalk(200, 'basic'); break;
        // 18. ToE 2
        case 17: Gen.cubeWalk(100, 'basic'); Gen.portal(3); Gen.shipTunnel(400, 3); Gen.portal(6); Gen.ufoSection(30); Gen.portal(7); Gen.waveSection(400, 3); break;
        // 19. GEO DOMINATOR
        case 18: Gen.cubeWalk(100, 'basic'); Gen.portal(8); Gen.cubeWalk(600, 'basic'); Gen.portal(4); Gen.cubePlatforms(20); break;
        ...
        // 21. FINGERDASH (Spider Sim)
        case 20: Gen.cubeWalk(100, 'basic'); Gen.portal(8); Gen.spiderSim(400); Gen.portal(5); Gen.spiderSim(400); break;
        // 22. DASH (Swing Sim)
        case 21: Gen.cubeWalk(50, 'basic'); Gen.portal(3); Gen.swingSim(600); Gen.portal(7); Gen.waveSection(500, 4); break;
        // 23. SEVEN SEAS
        case 22: Gen.cubeWalk(200, 'basic'); Gen.portal(3); Gen.shipTunnel(300, 6); Gen.portal(5); Gen.ballSection(300); break;
        // 24. VIKING ARENA
        case 23: Gen.cubeWalk(200, 'stereo'); Gen.portal(3); Gen.shipTunnel(400, 5); Gen.portal(6); Gen.ufoSection(20); break;
        // 25. AIRBORNE ROBOTS
        case 24: Gen.portal(8); Gen.cubeWalk(800, 'basic'); Gen.portal(3); Gen.shipTunnel(200, 5); break;
    }
...
function resetPlayer() {
    ...
    // Deadlocked starts with Wave
    if(gameState.levelIndex === 19) player.gamemode = 'WAVE';
    player.rotation = 0; player.dead = false; player.onGround = true;
    player.gravityScale = 1; gameState.cameraX = 0;
    modeDisplay.innerText = player.gamemode; crashFlash.classList.remove('flash-active');
}
